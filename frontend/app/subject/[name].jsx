...
export default function SubjectScreen() {
  ...
  const subjectName = params.name || "Maths";
  ...
  const chapters = CHAPTERS[sData.name] || CHAPTERS.Maths;
  ...
  return (
    <div style={{ backgroundColor: C.bg, minHeight: "100vh", paddingBottom: 40 }}>
      {/* Keeping bottom padding small since it's not a tab screen */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "48px 24px 16px", borderBottom: `1px solid ${C.bdr}`, position: "sticky", top: 0, backgroundColor: C.bg, zIndex: 10 }}>
        ...
        <div style={{ textAlign: "center" }}>
          <h1 style={{ fontWeight: 900, fontSize: 20, margin: 0, color: C.text }}>{sData.icon} {subjectName}</h1>
      ...
      <div style={{ padding: "24px 24px 0" }}>
        <Tap onClick={() => router.push({ pathname: '/chapter/[name]', params: { name: chapters[0], subject: subjectName } })}
          style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
            <div>
              ...
              <p style={{ color: C.muted, fontSize: 13, margin: "6px 0 0" }}>{subjectName} · Class 9 · 5 Questions</p>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, paddingBottom: 24 }}>
          {chapters.map((ch, i) => {
            ...
            return (
              <Tap key={i} onClick={() => router.push({ pathname: '/chapter/[name]', params: { name: ch, subject: subjectName } })}
